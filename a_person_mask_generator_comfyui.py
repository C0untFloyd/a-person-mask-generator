...
class APersonMaskGenerator:
    ...
    def generate_mask(
        ...
        # Combine all masks
        combined_masks = torch.cat(tensor_masks, dim=0)

        # Apply region_mask as boundary if provided
        if region_mask is not None:
            # Ensure region_mask has the same shape as combined_masks
            ...
            if region_mask.shape[0] == 1 and combined_masks.shape[0] > 1:
                region_mask = region_mask.expand(combined_masks.shape[0], -1, -1)
            
            # Apply the boundary mask (multiply to zero out everything outside region_mask)
            combined_masks = combined_masks * region_mask
