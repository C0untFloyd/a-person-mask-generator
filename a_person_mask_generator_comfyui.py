...
class APersonMaskGenerator:
    ...
    def generate_mask(
        ...
        # Apply region_mask as boundary if provided
        if region_mask is not None:
            # Ensure region_mask has the same shape as masks_b
            # region_mask should be (batch, height, width) or (height, width)
            if region_mask.dim() == 2:
                region_mask = region_mask.unsqueeze(0)
            
            # Broadcast region_mask to match batch size if needed
            if region_mask.shape[0] == 1 and masks_b.shape[0] > 1:
                region_mask = region_mask.expand(masks_b.shape[0], -1, -1)
            
            # Move region_mask to the same device as masks_b
            if region_mask.device != masks_b.device:
                region_mask = region_mask.to(masks_b.device)
            
            # Apply the boundary mask (multiply to zero out everything outside region_mask)
            masks_b = masks_b * region_mask
